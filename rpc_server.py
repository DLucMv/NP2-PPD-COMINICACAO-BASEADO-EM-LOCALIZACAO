...
class RPCServer:
    def __init__(self, user, host='localhost', port=9000):
        self.user = user
    ...
    def receive_message(self, sender_name, message):
        ...
        print(f"[{self.user.name}] Mensagem recebida de {sender_name}: {message}")
        print(f"[{self.user.name}] Mensagem recebida: {message}")
        return True
